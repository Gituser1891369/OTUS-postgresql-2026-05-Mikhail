...
3. Конфигурация
   /opt/zookeeper/conf/
├── zoo.cfg              # Основной конфиг (Можно копировать как шаблон из zoo_sample.cfg можно просто создат руками.)
...
-1. На ноде 3 (текущий лидер) — останавливаем   (у зукипера нет прямой команды переключения, либо переключаем через системд, либо переключаем через скрипты. Но есть один ньюанс, если зукипер запущеy через скрипт, то и управлять через скрипт. Если через системд то лучше так же включать/ отключать через сервис)
