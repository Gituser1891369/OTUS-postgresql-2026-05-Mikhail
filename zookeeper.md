...
# 6. Скачиваем и устанавливаем внешнего клиента для удобства работы Moba Extern (Или любой другой клиент по желанию)
...
1. Бинарные файлы и скрипты:
   Запускаются по полному пути 
- /opt/zookeeper/bin/zkServer.sh start
- /opt/zookeeper/bin/zkServer.sh status


2. Основной пусть куда инсталлируется : /opt/zookeeper/bin/
- ├── zkServer.sh          # Главный скрипт управления
- ├── zkCli.sh             # Клиент
- ├── zkEnv.sh             # Переменные окружения
- └── zkCleanup.sh         # Очистка логов



3. Конфигурация
   /opt/zookeeper/conf/
- ├── zoo.cfg              # Основной конфиг (Можно копировать как шаблон из zoo_sample.cfg можно просто создать руками.)
- ├── zoo_sample.cfg       # Пример конфига
- ├── log4j.properties     # Настройки логирования
- └── configuration.xsl    # Стили для XML

4. Команды:
 - ./zkServer.sh start  Запуск ZooKeeper
 - ./zkServer.sh stop   Остановка ZooKeeper  (чаще использую классический систем д  systemctl stop zookeeper.server)
 - ./zkServer.sh restart Перезапуск          (чаще использую классический систем д  systemctl restart zookeeper.server)
 - ./zkServer.sh status Статус ноды (Leader/Follower)

5. Самые основные команды применяемые на практике:
   
- Быстрая проверка хелсчека  	 echo ruok | nc localhost 2181 → imok
- Роль в кластере	             echo stat | nc localhost 2181 | grep Mode
- Все метрики	                 echo mntr | nc localhost 2181
- Кто подключен	к кластеру     echo cons | nc localhost 2181
- Конфигурация	               echo conf | nc localhost 2181
- Размер данных	               echo dirs | nc localhost 2181


6. Пример: как переключить лидера с ноды 3 на ноду 2
   
- 1. На ноде 3 (текущий лидер) — останавливаем   (у зукипера нет прямой команды переключения, либо переключаем через systemd, либо переключаем через скрипты. Но есть один ньюанс, если зукипер запущеy через скрипт, то и управлять через скрипт. Если через systemd, то лучше так же включать / отключать через сервис)
 - /opt/zookeeper/bin/zkServer.sh stop     Либо systemctl stop zookeeper.service          
- 2. Ждем 5-10 секунд — кластер выберет нового лидера
- 3. Проверяем, кто стал лидером
 -  echo stat | nc localhost 2181 | grep Mode
- 4. Запускаем ноду 3 обратно — она станет follower
- /opt/zookeeper/bin/zkServer.sh start

7.Основные моменты: 
- Чтобы бы состоялись выборы лидера в кластере зукипера, нужно минимум два сервера.  Когда в составе 3 сервера в кластере можно проводить манипуляции только с одним сервером одновременно. Если одновременно уроним 2 сервера в кластере зукипера, то одна нода становится в статус unreachable  и подключенные кластера patroni начинают непрерывное голосование и смену лидера. В результате чего можем получить несогласованность в работе или дестрой кластера. Соблюдая бест - практис использования кластера Zookeeper  нужно распологать каждую ноду в своём цоде, и каждую ноду распологать на отдельном гипервизоре. Чтобы разом не потерять 2 ноды и не нанести большой урон продакшн среде.
